...
C:\Users\konynag\Desktop\의료IT 유채연\Visual18615032\A021_DB\Students.accdb;Persist Security Info=False;";
        ...
        private void ReadAndAddToListBox()
        {
            ...
            // DB에서 읽어오는 여러개의 레코드
            while (reader.Read()) // 하나의 문자열로 만들어서 써줄거임
            {
                string x = "";
                x += reader["ID"] + "\t";
                x += reader["SId"] + "\t";
                x += reader["SName"] + "\t";
                x += reader["Phone"];
                listBox1.Items.Add(x); // listBox에 쓸 때는 Items.Add를 써야함
            }
            reader.Close();
        }
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            comm = new OleDbCommand(sql, conn);
            if (comm.ExecuteNonQuery() == 1) // return 값이 1이면 성공
